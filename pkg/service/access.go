...
func RevokeAccessToDataset(ctx context.Context, id string) *APIError {
	...
	bqds, apierr := GetBigqueryDatasource(ctx, access.DatasetID, false)
	if apierr != nil {
		return apierr
	}
	...
	if err := accessManager.Revoke(ctx, bqds.ProjectID, bqds.Dataset, bqds.Table, access.Subject); err != nil {
		return NewAPIError(http.StatusInternalServerError, err, "revokeAccessToDataset(): failed to revoke access")
	}
...
func GrantAccessToDataset(ctx context.Context, input GrantAccessData) *APIError {
	...
	bqds, apierr := GetBigqueryDatasource(ctx, ds.ID, false)
	if apierr != nil {
	...
	if err := accessManager.Grant(ctx, bqds.ProjectID, bqds.Dataset, bqds.Table, subjWithType); err != nil {
...
func accessFromSQL(access gensql.DatasetAccess) *Access {
	return &Access{
		ID:              access.ID,
		Subject:         access.Subject,
		Granter:         access.Granter,
...
func GetUnrevokedExpiredAccess(ctx context.Context) ([]*Access, error) {
	expired, err := queries.ListUnrevokedExpiredAccessEntries(ctx)
	if err != nil {
		return nil, err
	}
